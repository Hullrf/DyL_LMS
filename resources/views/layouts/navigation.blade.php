<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-1 sm:-my-px sm:ms-10 sm:flex sm:items-center">
                    ...
                    @if(Auth::user()->esEstudiante() || !Auth::user()->tieneRol('Administrador'))
                    <x-nav-link :href="route('calificaciones.mis')" :active="request()->routeIs('calificaciones.mis')">
                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/></svg>
                        {{ __('Mis Notas') }}
                    </x-nav-link>

                    <x-nav-link :href="route('certificados.mis')" :active="request()->routeIs('certificados.mis')">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium leading-5 transition duration-150 ease-in-out"
                            :class="request()->routeIs('admin.*') ? 'border-blue-400 text-gray-900 focus:border-blue-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:text-gray-700 focus:border-gray-300'">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute left-0 mt-1 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-50" x-transition>
                            <a href="{{ route('admin.usuarios.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Usuarios</a>
                            <a href="{{ route('admin.auditoria.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Auditoría</a>
                        </div>
                    </div>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->esEstudiante() || !Auth::user()->tieneRol('Administrador'))
            <x-responsive-nav-link :href="route('calificaciones.mis')" :active="request()->routeIs('calificaciones.mis')">
                {{ __('Mis Notas') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('certificados.mis')" :active="request()->routeIs('certificados.mis')">
                {{ __('Certificados') }}
            </x-responsive-nav-link>
            @endif
